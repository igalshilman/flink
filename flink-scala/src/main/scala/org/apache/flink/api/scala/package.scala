...
package object scala {
  ...
  // using the Scala API
  implicit def createTypeInformation[T]: TypeInformation[T] =
    macro TypeUtils.createTypeInfo[T]
  ...
  private[flink] def wrap[R: ClassTag](set: JavaDataSet[R]) =
    new DataSet[R](set)
  ...
  ): TypeInformation[T] = funcOrInputFormat match {

    case rtq: ResultTypeQueryable[_] =>
      rtq.asInstanceOf[ResultTypeQueryable[T]].getProducedType
    case _ => typeInfo
  }

  private[flink] def fieldNames2Indices(typeInfo: TypeInformation[_],
  ...
  def createTuple2TypeInformation[T1, T2](
    ...
    new CaseClassTypeInfo[(T1, T2)](
      ...
      ): TypeSerializer[(T1, T2)] = {
        val fieldSerializers: Array[TypeSerializer[_]] =
          new Array[TypeSerializer[_]](getArity)
        for (i <- 0 until getArity) {
          fieldSerializers(i) = types(i).createSerializer(executionConfig)
        }

        new Tuple2CaseClassSerializer[T1, T2](
          classOf[(T1, T2)],
          fieldSerializers
        )
      }
